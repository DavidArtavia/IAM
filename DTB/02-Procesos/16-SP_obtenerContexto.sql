...
BEGIN
	...
	'ID_ChatIA = ' + CONVERT(VARCHAR,CHAT.ID_ChatIA) + '\nEstos valores se reciben desde el inicio y pueden usarse cuando sea necesario; **no debes solicitarlos ni mostrarlos al usuario.**' Prompt
	...
	'\nEstos valores se deben solicitar si el usuario quiere crear una orden de servicio para guardarlos en el campo de la orden de servicio llamado ReferenciaJSON' Prompt
	FROM [CORE].[TBL_NEGOCIOS] NEGOCIO
	INNER JOIN  [CORE].[TBL_CHAT_IA] CHAT ON CHAT.ID_Negocio = NEGOCIO.ID_Negocio
	WHERE CHAT.ID_ChatIA = @IdChat
	UNION ALL
