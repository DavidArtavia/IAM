USE [IAMDB]
GO
IF NOT EXISTS (SELECT 1 FROM sys.procedures WHERE name = 'SP_buscarOrdenServicio')
BEGIN
    EXEC('CREATE PROCEDURE CORE.SP_buscarOrdenServicio AS BEGIN SET NOCOUNT ON; END')
END
GO
...
BEGIN
    ...
    WHERE 
        ...
        AND (
            -- 1) JSON: al menos un par no vacío de @ReferenciaJSON coincide parcialmente
            (
