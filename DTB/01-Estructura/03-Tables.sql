...
(
	...
	ReferenciaJSON NVARCHAR(MAX) CHECK (ISJSON(ReferenciaJSON) > 0) NULL --Par�metros que la IA le va a solicitar cuando se haga una orden de servicio para este negocio
)
GO
...
(
	...
	ReferenciaJSON NVARCHAR(MAX) CHECK (ISJSON(ReferenciaJSON) > 0) NULL, --Referencias adicionales que se piden seg�n lo configurado para el negocio (Placa, a�o, marca)
...
--en el momento que se detecte lo que quiere hacer dentro de una conversaci�n, se guarda un registro ac� para darle seguinmiento
--adem�s dentro del sistema se valida una l�gica (CAPA APP) para entender todos los par�metros que son requeridos y guardarlos en otra tabla
--para darle seguimiento hasta tener todos los par�metros y ejecutar la acci�n
CREATE TABLE CORE.TBL_INTENCIONES_DETECTADAS (
    ...
    NombreIntencion NVARCHAR(100), --Mismo nombre que el m�todo que realiza la intenci�n en el API
    FechaDeteccion DATETIME DEFAULT GETDATE() NOT NULL,
	FechaEjecucion DATETIME DEFAULT GETDATE() NULL,
	Resultado NVARCHAR(255) NULL-- �xito, error, validaci�n fallida y cualquier otra vara que salga (si es error se ampliara en los LOGs
)
GO
...
CREATE TABLE UTIL.TBL_ALERTAS
(
	COD_ALERTA VARCHAR(10) NOT NULL, --PK
