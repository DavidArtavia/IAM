...
-- Creación: 18/07/2025
-- Descripción: Obtener transacciones por cuenta (Tipo = 'Cuenta' y NumReferencia = CuentaId)
-- =============================================

-- Stub: Crear procedimiento si no existe
IF NOT EXISTS (
    SELECT 1
    FROM sys.procedures
    WHERE name = 'SP_obtenerTransaccionesPorCuenta'
)
BEGIN
    EXEC('CREATE PROCEDURE [CORE].[SP_obtenerTransaccionesPorCuenta] AS BEGIN SET NOCOUNT ON; END')
END
GO
