...
import { AsyncClientSelect, ConfirmModal, GenericDataTable, GenericFormModal, InfoPanel, ItemsOrdenDeServicioModal, LoadingPanel, } from "@/components";
import { RESTRICCIONES, STATUS_ORDEN_SERVICIO_OPTIONS, STATUS_TBL, } from "@/constants";
import { DTO_OrdenServicio } from "@/models";
import { ordenesService } from "@/services";
import { keysInfoModalOrdenDeServicio, columnKeysOrdenDeServicio, dateHelpers, errorHelpers, labelMapOrdenDeServicio, notificationHelpers, ordenServicioFormEditFields, } from "@/utils";
import { useApp } from '@/hooks/useApp';
import { useEffect, useMemo, useState } from "react";
import AsyncSelect from "react-select/async";
const generateSafeKey = (name) => name
    .trim()
    .toLowerCase()
    .replace(/\s+/g, "_")
    .replace(/[^\w_]/g, "");
export const OrdenDeServicio = () => {
    ...
    const [dataToItemsOrder, setDataToItemsOrder] = useState(null);
    //#endregion
    //#region 🗑 Confirmación
    ...
    useEffect(() => {
        ...
        const sub = ordenesService
            ...
            .subscribe({
            next: (res) => setOrdenes(res.resultado || []),
    ...
    const handleSelectBusiness = (neg) => {
    ...
    const handleCancelAdd = () => {
        setConfirmModalMessage("¿Estás seguro de que deseas cancelar la nueva orden?");
        setConfirmContext("cancelAdd");
        setIsConfirmOpen(true);
    };
    //#endregion
    //#region 🛠️ Editar
    const normalizeIncomingDate = (raw) => {
        if (!raw)
            return null;
    ...
    const handleDelete = (row) => {
        setConfirmModalMessage(`¿Estás seguro de que deseas eliminar la orden ${row.notaOrdenServicio}?`);
    ...
    const handleConfirmDelete = (action) => {
        if (action && orderToDelete) {
            ...
            setOrdenes((prev) => prev.map((o) => o.iD_OrdenServicio === sanitized.iD_OrdenServicio ? sanitized : o));
    ...
    const buildRefFields = (item) => item.referenciaJSON?.map((r, idx) => ({
        key: generateSafeKey(r.nombre),
        label: r.nombre,
        type: "custom",
        renderer: () => (_jsx("input", { className: "form-control", value: item.referenciaJSON?.[idx].valor || "", onChange: (e) => {
    ...
    const newFormFields = [
        ...ordenServicioFormEditFields,
        {
            ...
            type: "custom",
            required: true,
            renderer: ({ onChange }) => (_jsx(AsyncClientSelect, { value: selectedClientOption, onChange: (opt) => {
        ...
        },
        {
            ...
            renderer: ({ value, onChange }) => {
                const selectedOption = value?.iD_Estado
                    ...
                    : null;
                return (_jsx(AsyncSelect, { cacheOptions: true, defaultOptions: STATUS_ORDEN_SERVICIO_OPTIONS, placeholder: "Seleccione un estado", value: selectedOption, onChange: (opt) => onChange({ iD_Estado: opt?.value, nombre: opt?.label }), loadOptions: async (inputValue) => STATUS_ORDEN_SERVICIO_OPTIONS.filter((opt) => opt.label.toLowerCase().includes(inputValue.toLowerCase())) }));
            },
        },
    ...
    const customRenderers = {
        fechaOrdenServicio: (val) => val ? new Date(String(val)).toLocaleDateString() : "",
        fechaEstimadaEntrega: (val) => val ? new Date(String(val)).toLocaleDateString() : "",
    };
    //#endregion
    //#region 🧠 Memo tabla
    const { data, labelMap, modalFields } = useMemo(() => {
        ...
        const ordenesActivas = ordenes.filter((o) => o.estado?.iD_Estado !== STATUS_TBL.ORDER_SERVICE.DELETED);
        const prepared = ordenesActivas.map((o) => {
            const copy = { ...o };
            o.referenciaJSON?.forEach((r) => (copy[referenceMap.get(r.nombre)] = r.valor));
            return copy;
        });
        ...
        const staticKeys = keysInfoModalOrdenDeServicio;
    ...
    //#endregion
    // #region 🧩 Render
    return (_jsxs("div", { className: "row p-4 gx-0", children: [state.negocio == null, loading ? (_jsx(LoadingPanel, { msj: "Cargando \u00F3rdenes de servicio, por favor espere..." })) : selectedBusiness ? (_jsx(GenericDataTable, { title: "\u00D3rdenes de Servicio", columnKeys: columnKeysOrdenDeServicio, labelMap: labelMap, data: data, onAdd: handleAddNew, onEdit: handleEdit, onDelete: handleDelete, disableButtonAdd: disableButtonAdd, includeEstadoColumn: true, includeReferenceColumn // Si se quiere mostrar la columna de referenciasJson
                : true, modalInfoFields: modalFields, showItemsButton: true, datekeys: [
                    ...
                    setDataToItemsOrder(rowData);
                }, customRenderers: customRenderers })) : (_jsx(InfoPanel, { msj: "Seleccione un negocio para ver las \u00F3rdenes de servicio" })), _jsx(GenericFormModal, { title: "Registrar Orden", show: isFormOpen, onHide: handleCancelAdd, data: formData, setData: setFormData, onSubmit: handleSave, fields: newFormFields }), _jsx(GenericFormModal, { title: "Editar Orden de Servicio", show: showEditForm, onHide: () => setShowEditForm(false), data: editData, setData: setEditData, onSubmit: handleSaveEdit, fields: editFormFields }), _jsx(ItemsOrdenDeServicioModal, { open: showItemsOrdenFormModal, onHide: () => setShowItemsOrdenFormModal(false), rowData: dataToItemsOrder || new DTO_OrdenServicio() }), _jsx(ConfirmModal, { show: isConfirmOpen, confirmMessage: confirmModalMessage, onAction: (action) => confirmModalAcion(action) })] }));
};
