...
export const Monitor = () => {
        ...
        itemsOrdenesService.actualizarItemsOrdensDeServicio(item).subscribe({
            next: (res) => {
                if (!res.tipoRespuesta) {
                    notificationHelpers.errorAlert(res.mensaje);
                }
            },
            error: (err) => errorHelpers.serverError(err),
    ...
    useEffect(() => {
        ...
        const construirConexion = () => new signalR.HubConnectionBuilder()
            ...
            .build();
        const extraerYRenovarToken = async (payloadText) => {
        ...
        const handleMensaje = (msg) => {
            ...
            if (isItemOrdenServicio(msg)) {
                ...
                setOrdenes((prev) => {
                    const idx = prev.findIndex((o) => o.iD_OrdenServicio === msg.iD_OrdenServicio);
                    /* —— 1 · Eliminar si el estado es 19 —— */
        ...
        const iniciarConexion = async () => {
            if (abortedRef.current || intentoRef.current)
                return;
    ...
    useEffect(() => {
        ...
        const sub = monitorService
            ...
            .subscribe({
            next: (res) => {
                setOrdenes(res.resultado[0] || []);
                setItems(res.resultado[1] ||
                    []);
            },
            error: (err) => errorHelpers.serverError(err),
            complete: () => setLoading(false),
    ...
    const handleCreateCount = (countSelected) => {
        // Aquí se maneja la logic para el modal de crear cuenta
        console.log("Crear cuenta para:", countSelected);
    };
    //#endregion
    return (_jsxs("div", { children: [_jsx("div", { className: "row p-4 gx-0", children: loading && _jsx(LoadingPanel, { msj: "Cargando, por favor espere..." }) }), _jsxs("div", { id: "kt_content_container", className: "container-xxl", children: [_jsx("div", { className: "d-flex flex-wrap flex-stack pt-10 pb-8", children: _jsxs("h3", { className: "fw-bolder my-2", children: [_jsx("span", { style: { marginRight: "5px", marginBottom: "-5px" }, className: `badge badge-circle ${estadoConexion === "Conectado"
                                        ? " badge-success"
                                        : " badge-danger"}` }), selectedBusiness?.nombreNegocio, _jsx("span", { className: "fs-6 text-gray-400 fw-bold ms-1", children: estadoConexion })] }) }), _jsxs("div", { className: "row g-9", children: [_jsx(OrdenesSeccion, { titulo: "Nuevo", colorBarra: "bg-secondary", estado: STATUS_TBL.ORDER_SERVICE.NEW, ordenes: ordenes, items: items, onAvanceChange: handleCheckboxChange, onEstadoChange: cambiarEstadoOrdenServicio }), _jsx(OrdenesSeccion, { titulo: "En proceso", colorBarra: "bg-primary", estado: STATUS_TBL.ORDER_SERVICE.IN_PROCESS, ordenes: ordenes, items: items, onAvanceChange: handleCheckboxChange, onEstadoChange: cambiarEstadoOrdenServicio }), _jsx(OrdenesSeccion, { titulo: "Completado", colorBarra: "bg-success", estado: STATUS_TBL.ORDER_SERVICE.COMPLETED, ordenes: ordenes, items: items, onAvanceChange: handleCheckboxChange, onEstadoChange: cambiarEstadoOrdenServicio, onClickCreateCount: handleCreateCount }), _jsx(OrdenesSeccion, { titulo: "En espera", colorBarra: "bg-warning", estado: STATUS_TBL.ORDER_SERVICE.PENDING, ordenes: ordenes, items: items, onAvanceChange: handleCheckboxChange, onEstadoChange: cambiarEstadoOrdenServicio })] })] })] }));
};
