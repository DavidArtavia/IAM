        ...
        public async Task<DTO_Respuesta> actualizarOrdenServicio(DTO_OrdenServicio ordenServicio)
        {
            try
            {
                ...
                using (SqlCommand sqlcmd = new(query, this.GetObjConexion()))
                {
                    ...
                    sqlcmd.Parameters.Add("@ID_OrdenServicio", SqlDbType.Int).Value = ordenServicio.ID_Cliente;
                    sqlcmd.Parameters.Add("@ID_Cliente", SqlDbType.Int).Value = ordenServicio.Estado.ID_Estado;
